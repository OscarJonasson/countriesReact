...
import { addFavorite, removeFavorite } from "../../features/countries/cartSlice";
...
const CountriesCard = ({ country }) => {
  ...
  const populationRounding = () => {
    if (country?.population < 1000) return country?.population;
    const population = (country?.population / 1000000).toPrecision(2);
    return population >= 1 ? population + "M" : Math.round(population * 1000) + "K";
  };
  ...
  return (
    ...
    >
      ...
      <Card.Body>
        {favorites.map((name) => name.name.common).includes(country.name?.common) ? (
          <button
        ...
        <div className={classes.info__seeMore}>
          <Link to={`/countries/${country?.name?.common}`.toLowerCase()} state={country}>
